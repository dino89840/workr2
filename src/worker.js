...
function buildDownloadLinks(filename, accounts) {
  const links =[];
...
async function buildSignedHeaders(account, method, objectKey, queryString, extraHeaders, payloadHash) {
  ...
  const allHeaders = { ...extraHeaders, host, "x-amz-content-sha256": payloadHash, "x-amz-date": amzDate };
...
async function uploadSimplePut(account, objectKey, body, contentType) {
...
async function initiateMultipart(account, objectKey, contentType) {
...
async function completeMultipart(account, objectKey, uploadId, parts) {
...
async function abortMultipart(account, objectKey, uploadId) {
  try {
...
async function streamingMultipartUpload(account, objectKey, reader, contentType) {
...
async function bufferMultipartUpload(account, objectKey, body, contentType) {
  const uploadId = await initiateMultipart(account, objectKey, contentType);
  try {
...
async function uploadToBothR2(accounts, objectKey, body, contentType) {
  ...
  for (let idx = 0; idx < accounts.length; idx++) {
    ...
    } catch (err) { errors.push(`${account.label}: ${err.message}`); }
  }
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/remote-upload") {
      try {
        ...
        let pathName = "file";
        try { pathName = new URL(remoteUrl).pathname; } catch (e) {}
